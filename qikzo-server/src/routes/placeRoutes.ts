...
import { createPlaceSchema, updatePlaceSchema } from '../validators/userValidators';

const router = Router();
router.get('/', requireAuth, placeController.list);
router.post('/', requireAuth, validate(createPlaceSchema), placeController.create);
router.patch('/:id', requireAuth, validate(updatePlaceSchema), placeController.update);
