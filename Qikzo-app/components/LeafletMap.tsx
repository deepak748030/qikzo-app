...
type Props = {
  ...
  // real roads (matches the rider app's post-accept map look).
  riderLocation?: LatLng | null;
  // Vehicle kind booked by the customer. When provided along with
...
<script>
  ...
  var riderAnim = null;
  var riderHeading = 0; // last known bearing (deg, 0=north)
  var riderVehicle = ${vehicleKind ? `'${vehicleKind}'` : 'null'};
  // Top-down photographic vehicle silhouettes (user-supplied top-view assets),
  // inlined as base64 data URIs so the WebView can render them without any
  // network fetch or asset resolution. Head-of-vehicle orientation in the
  // source images is: bike → top (0°), auto → bottom (180°), car → bottom (180°).
  ...
  function buildRiderIcon(vehicle, headingDeg){
    ...
    // Fallback: pulsing blue dot (unknown vehicle).
    return L.divIcon({
      className:'',
...
<div style="position:absolute;inset:0;border-radius:50%;background:${colors.primary};border:3px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.35);"></div>\
  ...
  function moveRider(lat, lng){
    ...
    // Update heading only if there was meaningful motion (>~1.5m).
  ...
  ${riderLocation && pickup ? `
    ...
    (function(){
      ...
      fetch(url).then(function(r){return r.json();}).then(function(d){
        ...
        try{ map.fitBounds(L.polyline(coords).getBounds(), { padding:[60,60], maxZoom:16 }); }catch(e){}
      }).catch(function(){
        ...
        try{ map.fitBounds(L.latLngBounds(pts), { padding:[60,60], maxZoom:16 }); }catch(e){}
      });
    })();
  ` : ''}
  ...
  function setLiveRiders(list){
    ...
    // Auto-fit: when there is no pickup->drop route drawn yet, zoom the map
    // so that both the pickup pin and every live rider are visible together.
    ${pickup ? `
    if(!routeLineBg){
      var pts=[[${pickup.lat},${pickup.lng}]];
      (list||[]).forEach(function(r){ if(typeof r.lat==='number' && typeof r.lng==='number') pts.push([r.lat,r.lng]); });
      if(pts.length>=2){
        try{ map.fitBounds(L.latLngBounds(pts), { padding:[60,60], maxZoom:16, animate:true }); }catch(e){}
      }
    }
    ` : ''}
  }
...
export default function LeafletMap({
  ...
  // Swap the rider marker's vehicle emoji live when the booked category
  // becomes known (e.g. after `rider accepted`) without rebuilding the map.
  React.useEffect(() => {
    if (!ref.current) return;
